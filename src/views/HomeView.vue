...
<template>
    <div class="home-view">
        <v-container class="py-4">
            <!-- Total Expenses Card -->
            <v-card color="primary" variant="elevated" class="mb-4">
                <v-card-text class="text-center py-6">
                    <div class="text-overline text-white opacity-80">
                        {{ t('home.totalExpenses') }}
                    </div>
                    <div class="text-h3 font-weight-bold text-white">
                        <v-progress-circular v-if="isLoading" indeterminate color="white" size="32" />
            ...
            <v-card elevation="2">
                ...
                <v-card-actions v-if="recentExpenses.length > 0" class="pa-4">
                    <v-btn color="primary" variant="text" block @click="router.push('/history')">
                        {{ t('home.viewHistory') }}
                    </v-btn>
                </v-card-actions>
            </v-card>

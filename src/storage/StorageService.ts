 ...
 * ```
 */
export async function storeFile(data: string, mimeType: string, options: StorageOptions = {}): Promise<StoreResult | StorageError> {
    if (!isInitialized) {
    ...
    try {
        ...
        const filename = options.filename ? `${options.filename}${extension}` : `${fileId}${extension}`;
        ...
        return createError(StorageErrorCode.IO_ERROR, `Failed to store file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
 ...
 * ```
 */
export async function retrieveFile(fileId: string, createObjectUrl: boolean = false): Promise<RetrieveResult | StorageError> {
    if (!isInitialized) {
        return createError(StorageErrorCode.NOT_INITIALIZED, 'Storage not initialized. Call initializeStorage() first.');
    }
    ...
    try {
        ...
        return createError(StorageErrorCode.IO_ERROR, `Failed to retrieve file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
...
export async function deleteFile(fileId: string): Promise<DeleteResult | StorageError> {
    ...
    try {
        ...
        return createError(StorageErrorCode.IO_ERROR, `Failed to delete file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
...
export async function listFiles(options?: {
    ...
    try {
        ...
        return createError(StorageErrorCode.IO_ERROR, `Failed to list files: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}

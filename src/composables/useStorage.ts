...
export function useStorage() {
     ...
     */
    async function saveFile(file: File, options?: StorageOptions): Promise<FileMetadata | null> {
        if (!isReady.value) {
     ...
     * @returns FileMetadata if successful, null otherwise
     */
    async function saveBase64(base64Data: string, mimeType: string, options?: StorageOptions): Promise<FileMetadata | null> {
        if (!isReady.value) {
            const initialized = await initialize();
            if (!initialized) {
                return null;
     ...
     */
    async function loadFile(fileId: string, withObjectUrl: boolean = false): Promise<{ metadata: FileMetadata; data: string; objectUrl?: string } | null> {
        if (!isReady.value) {
            const initialized = await initialize();
            if (!initialized) {
                return null;
     ...
     */
    async function getFiles(options?: { directory?: string; mediaType?: MediaType }): Promise<FileMetadata[]> {
        if (!isReady.value) {
            const initialized = await initialize();
            if (!initialized) {
                return [];

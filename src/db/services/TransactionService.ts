...
export class TransactionService extends BaseCRUDService<Transaction, string> {
    ...
    async findWithFilters(filters: TransactionFilters): Promise<Transaction[]> {
        ...
        return qb.orderBy('transaction.transactionDate', 'DESC').addOrderBy('transaction.createdAt', 'DESC').getMany();
    }
     ...
     */
    async getTotalByType(ledgerId: string, transactionType: TransactionType, currency: string, startDate?: string, endDate?: string): Promise<number> {
